...
Section~\ref{sec:problem} cast Ramsey lower-bound construction as a pseudo-Boolean violation-minimization problem. We now turn this into a continuous Frank-Wolfe pipeline whose subsections accumulate from relaxation to a verified binary witness.
...
The objective is a multilinear polynomial of degree $\binom{\max(r,s)}{2}$ in $m$ variables, with gradients computable whenever the relevant vertex-set families can be enumerated. It is supermodular on the hypercube --- each clique term has nonnegative mixed partial derivatives in the variables it contains --- but non-monotone: the $K_r$ terms are increasing in the edge variables whereas the independent-set terms are decreasing.
