...
\begin{abstract}
    Establishing Ramsey number lower bounds constitutes a well-studied class of hard combinatorial feasibility problems. Known lower-bound witnesses have accumulated over decades through problem-specific discrete heuristics, with several key values remaining out of reach for generic methods. We replace direct combinatorial search by continuous relaxation followed by exact verification. Casting the construction problem as minimization of a pseudo-Boolean polynomial violation count turns satisfiability into optimization; relaxing the binary constraints to the unit hypercube then yields a smooth multilinear objective.
    
    Frank-Wolfe performs surprisingly well on this relaxation: its linear minimization oracle over the hypercube returns binary vertices, driving reliable progress despite the objective being non-monotone supermodular. A conditional-expectations rounding converts the fractional solution to a binary candidate without increasing the violation count.
...
unit hypercube yields a differentiable problem accessible to first-order methods. For structured
objectives this approach has tight theory: Continuous Greedy, for example, attains a $(1-1/e)$
approximation for monotone submodular maximization under matroid constraints by linear
optimization over the feasible polytope~\cite{vondrak2008optimal,calinescu2011maximizing}.
No comparable $(1-1/e)$-type guarantee is known in the complementary regime of non-monotone supermodular objectives. Generic
nonconvex Frank-Wolfe~\cite{braun2022conditional} supplies only stationarity guarantees, but on
hypercube relaxations its linear oracle still returns combinatorial vertices. The question is
whether this geometry can help produce certifiable discrete witnesses. Ramsey lower-bound search
is a natural test case: it is well studied, has a long record of specialized constructive
methods, and every candidate graph can be checked exactly.
