...
We treat the choice of surrogate as a design decision rather than a theorem-level necessity. The vanilla probability loss is a natural baseline, while the log-barrier objective remains useful as a principled way to emphasize high-probability near-violations. A systematic ablation separating the surrogate from the optimizer and batching strategy is left for future work.
...
\begin{table}[t]
  ...
  \caption{Results obtained through our method. Blue cells indicate lower bounds that are known to be optimal, yellow cells indicate lower bounds that are not yet known to be optimal and red cells indicate lower bounds that are known to be suboptimal, according to the current small-Ramsey survey and graph database~\cite{radziszowski2026smallramsey,mckayramseydata}.}
