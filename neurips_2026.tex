...
The objective is a multilinear polynomial of degree $\binom{\max(r,s)}{2}$ in $m$ variables, so gradients can be computed explicitly whenever the relevant vertex-set families can be enumerated. The relaxation exposes informative descent directions and lets the optimizer exploit partial progress before iterates reach a hypercube vertex.
...
\]
so at least one of the two endpoints has value no larger than the current conditional expectation. Iterating this argument over all edges produces a binary witness $\widehat{x} \in \{0,1\}^m$ with
\[
    M(\widehat{x}) \;\leq\; M(x).
\]
Since $M(\widehat{x})$ is a nonnegative integer, any fractional point with $M(x) < 1$ is guaranteed to round to a Ramsey witness with $M(\widehat{x}) = 0$. This threshold is sufficient, not necessary: conditional-expectation rounding can still succeed from points with larger raw expected violation count, and the final certificate does not rely on the fractional objective value. We verify $\widehat{x}$ against the original Ramsey constraints by the clique-checking procedure described in \cref{sec:problem}.
