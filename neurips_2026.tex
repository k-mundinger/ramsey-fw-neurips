...
For the exact-gradient version on the smooth raw objective $M$, this update also falls under standard nonconvex Frank-Wolfe theory: under the usual smoothness and step-size assumptions, the minimum Frank-Wolfe gap among the first $T$ iterates decays as $O(T^{-1/2})$ \cite{lacostejulien2016convergence,braun2022conditional}. This is only a stationarity guarantee, not a guarantee of finding a Ramsey witness, and our reported pipeline also uses surrogate losses and stochastic clique batches. Its role is therefore to position the update as a principled projection-free baseline; successful lower-bound certificates still come from conditional-expectation extraction followed by exact verification.
