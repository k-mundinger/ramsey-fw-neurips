...
\begin{enumerate}
    \item We develop a projection-free Frank-Wolfe pipeline (closed-form LMO, stochastic clique batching, conditional-expectations rounding with exact verification) for the multilinear hypercube relaxation of the Ramsey violation objective, run without algebraic restrictions on the graph structure.
...
An $n$-vertex graph $G$ is an $(r,s,n)$-Ramsey graph if it contains no clique of size $r$ and no independent set of size $s$, i.e., $\omega(G) < r$ and $\alpha(G) < s$. The Ramsey number $R(r,s)$ is the least $n$ for which no such graph exists; the case $r = s$ is called diagonal and $r \neq s$ off-diagonal. Proving the lower bound $R(r,s) > n$ amounts to exhibiting a single $(r,s,n)$-Ramsey graph, while the upper bound $R(r,s) \leq n$ requires ruling out all such graphs at order $n$; we address the lower-bound construction side. Verifying a candidate $G$ reduces to clique-existence tests; for fixed $r,s$ this is polynomial in $n$ by brute enumeration and well-handled in practice by solvers such as Cliquer~\cite{niskanen2003cliquer}. The bottleneck is search: the graph space is exponential in $n$ even after quotienting by isomorphism, ruling out exhaustive construction at all but the smallest orders.
...
Having set up Ramsey lower-bound construction as a certified search problem, we now cast it as pseudo-Boolean violation minimization and turn it into a continuous Frank-Wolfe pipeline. The subsections accumulate from the discrete formulation to relaxation, stochastic optimization, and finally a verified binary witness.
...
The raw expected-count objective $M$ is used for rounding and certification, but in practice it can provide a weak optimization signal. Each clique contribution is a high-order product of edge probabilities, so its gradient can remain small unless the corresponding vertex set is already very close to fully monochromatic. A first-order method can then spend many iterations reducing diffuse background risk while paying insufficient attention to the local patterns most likely to become forbidden cliques after rounding.
...
\]
for the corresponding monochromatic probabilities under the independent-edge interpretation. These probabilities are the right quantities for certification, but not necessarily the best quantities to optimize directly. The issue is geometric decay: a $k$-vertex clique contains $\binom{k}{2}$ edges, so even if every relevant edge has probability $q$, the clique probability is only $q^{\binom{k}{2}}$. For example, a $5$-clique with all ten edge probabilities equal to $0.9$ contributes only $0.9^{10}$. Optimizing the raw count can therefore underweight large, consistently biased vertex sets that are plausible sources of rounded violations.
...
\end{equation}
The surrogate changes only the search loss. The logarithmic transformation treats each clique as an individual soft constraint, with a penalty that grows rapidly as its monochromatic probability approaches $1$. Equivalently, relative to the raw product term, the scalar part of the gradient is amplified by the factor $(1-p)^{-1}$ for a clique with monochromatic probability $p$. Despite the superficial resemblance, this is not a classical interior-point log-barrier: a barrier method repels iterates from the boundary of the feasible set, whereas here it is the forbidden monochromatic subgraphs that are repelled. The values $0$ and $1$ on individual edges are not penalized; the optimizer can move toward discrete edge colorings as long as no clique becomes fully monochromatic in the process.

\paragraph{Gradient clipping.}
The logarithmic surrogate is undefined when any clique is monochromatic with probability $1$. To keep the search loss finite, we replace each monochromatic clique probability $p$ by $\min(p,1-\varepsilon)$ before evaluating $-\log(1-p)$, for a small $\varepsilon>0$. The forward objective is then finite, and the largest per-clique penalty is controlled by $-\log \varepsilon$. The gradient convention through this clipping step, including the straight-through variant used as a search heuristic, is described in \cref{sec:algorithm-run-settings}.
...
We treat the choice of surrogate as a design decision rather than a theorem-level necessity. The vanilla probability loss $M$ remains the certifying objective; the log-barrier surrogate is used to make the search more responsive to near-monochromatic local structure.
...
\end{equation}
where $\gamma_t \in [0,1]$ is a step size. The schedule used in the experiments is reported in \cref{sec:algorithm-run-settings}.
...
All experiments reported below use this vanilla Frank-Wolfe update with a standard diminishing step-size schedule. Pairwise and away-step variants are straightforward active-set baselines, but a careful comparison at this scale is left to future work.
...
\[
    M(\widehat{x}) \;\leq\; M(x).
\]
Since $M(\widehat{x})$ is a nonnegative integer, any fractional point with $M(x) < 1$ is guaranteed to round to a Ramsey witness with $M(\widehat{x}) = 0$. This threshold is sufficient, not necessary: conditional-expectation rounding can still succeed from points with larger raw expected violation count, and the final certificate does not rely on the fractional objective value. We verify $\widehat{x}$ against the original Ramsey constraints by the clique-checking procedure described in \cref{sec:problem}. Thus the search procedure is heuristic, but every lower bound reported below is certified only after deterministic extraction and exact verification.
...
The cases the method does not yet solve are equally informative. In particular, the current runs do not produce a $35$-vertex witness for $R(4,6)$. Larger near-diagonal cases such as $R(4,7)$ and $R(5,6)$ involve substantially larger clique families and fall outside the present experimental range. These limitations are consistent with the broader picture that the method is currently strongest on diagonal and near-diagonal cases.

\paragraph{Comparison with prior work.}
Taken together, these results show that Frank-Wolfe style continuous optimization is competitive with the ad-hoc vertex-by-vertex extension pipelines previously used for these problems, while running directly on the target instance. The strongest evidence is currently on diagonal and near-diagonal cases. Recent AlphaEvolve-based work improves or matches many classical lower bounds and is especially strong on larger off-diagonal instances~\cite{nagda2026reinforcedgenerationcombinatorialstructures}. The present method occupies a different regime: it runs vanilla Frank-Wolfe directly on the violation objective, without program synthesis or algebraic/circulant restrictions on the edge variables, and already recovers nontrivial witnesses, including $R(5,5) \geq 43$, which AlphaEvolve does not report.


\section{Conclusion and Outlook}

We presented a continuous-optimization route to Ramsey lower-bound construction: relaxation of the pseudo-Boolean violation objective to the hypercube, Frank-Wolfe with a vertex-returning oracle, and conditional-expectations rounding for binary extraction. The resulting objective is nonconvex and non-monotone supermodular, so the continuous optimizer is used as a search heuristic rather than as a source of global guarantees. Independently, the raw multilinear objective supports exact extraction: it has no integrality gap at the level of global optima, and conditional-expectation rounding converts any fractional candidate into a binary graph without increasing the raw violation count. Final claims are certified by exact verification, not by the continuous optimizer alone.

The experiments give a positive answer within the tested range to the empirical question posed in the introduction: first-order optimization on the hypercube relaxation can find useful discrete Ramsey witnesses in practice. This pipeline recovers a substantial part of the small-Ramsey lower-bound table and repeatedly finds $42$-vertex witnesses for $R(5,5) \geq 43$, without imposing circulant, algebraic, or other structural restrictions on the graph. The multiplicity of recovered witnesses, especially for $R(5,5;42)$ and $R(4,4;15)$, shows that the search dynamics reach multiple basins leading to valid certificates rather than a single isolated construction. At the same time, the method is not yet a universal Ramsey-search engine: nearby cases such as $R(4,6)$ remain unsolved by the current implementation, and larger off-diagonal instances introduce much larger clique families.

Several directions follow from these limitations. On the algorithmic side, active-set variants of Frank-Wolfe, alternative step-size rules, and initialization priors could change which witness basins are reached. On the modeling side, sharper ablations of the raw objective, log-barrier surrogate, clipping convention, and batching scheme would clarify which ingredients are essential. The same template also has concrete Ramsey-theoretic extensions: multicolor problems replace each edge variable by a simplex vector whose LMO still chooses one color per edge; ordered and canonical variants change the indexed forbidden-pattern family; and Ramsey multiplicity changes the target from zero violations to minimizing a certified monochromatic-clique count.
...
The appendix collects implementation details, run settings, the batching scheme, and supplementary visualizations of recovered colorings. It also includes extended witness statistics and additional instance-by-instance results that are useful for reproducibility but not essential to the main narrative.

\subsection{Algorithm summary and run settings}\label{sec:algorithm-run-settings}

The computational pipeline used in the experiments is the following.
\begin{enumerate}
    \item Fix an instance $(r,s,n)$ and form the edge-variable representation $x \in [0,1]^{\binom{n}{2}}$.
    \item Choose a search loss $F$: either the raw violation objective $M$ or a clique-wise surrogate such as \cref{eq:log_barrier}, with the clipping convention specified below.
    \item Initialize a fractional edge vector $x_0$ according to the stated initialization rule.
    \item For each epoch, shuffle the relevant vertex-set families, form mini-batches as in the batching scheme below, and run vanilla Frank-Wolfe updates using the gradient or gradient estimator of the batched search loss.
    \item Apply conditional-expectation extraction to selected fractional iterates, always using the raw violation objective $M$.
    \item Verify each extracted binary graph by exact clique checking in both colors. Only graphs passing this final check are counted as certificates.
\end{enumerate}


\paragraph{Clipping convention.}
For a clique-wise probability $p$, the clipped log surrogate evaluates the forward term at $\tilde{p}=\min(p,1-\varepsilon)$. With the ordinary clamp, the derivative with respect to $p$ is set to zero when $p>1-\varepsilon$. In the straight-through variant, the forward pass still uses $\tilde{p}$, but the backward pass treats the clipping map as the identity. This keeps the scalar logarithmic factor bounded by $1/\varepsilon$ while preserving the product-gradient direction of the unclipped term. We use this only as an optimization heuristic; certification is unaffected because extraction and verification use $M$ and exact clique checks.

\paragraph{Hyperparameters.}
For the headline $R(5,5)$ experiments, the best results were obtained with initial Frank-Wolfe step sizes between $0.03$ and $0.08$, followed by a linearly decaying step-size schedule. We used $5000$ epochs, with each epoch making one pass through all cliques using batch size $16384$, and clipping threshold $\varepsilon=10^{-6}$ for the log surrogate. These choices affect search efficiency and the distribution of witnesses recovered, but not the validity of any reported lower bound after exact verification.
