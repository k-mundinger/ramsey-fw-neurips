...
\begin{abstract}
    Establishing Ramsey number lower bounds by finding $n$-vertex graphs that avoid large cliques and large independent sets constitutes a well-studied class of hard combinatorial feasibility problems. Known lower-bound witnesses have accumulated over decades through problem-specific discrete heuristics, with several key values remaining out of reach for generic methods. Recent LLM-driven evolutionary search using AlphaEvolve (Nagda et al., 2026) follows the same pattern: it obtained new off-diagonal bounds but stayed entirely within discrete search.

    We replace direct combinatorial search by continuous relaxation followed by exact verification. Casting the construction problem as minimization of a pseudo-Boolean polynomial violation count and relaxing the binary constraints to the unit hypercube yields a smooth multilinear objective. Frank-Wolfe performs surprisingly well on this relaxation: its linear minimization oracle over the hypercube returns binary vertices, driving reliable progress despite the objective being non-monotone supermodular. The connection to Continuous Greedy, which exploits the same linear oracle for monotone submodular maximization, offers some intuition as to why. A conditional-expectations rounding converts the fractional solution to a binary one without increasing the violation count. This pipeline recovers a significant portion of the known lower-bound table, most notably finding $42$-vertex graphs certifying $R(5,5) \geq 43$, a bound standing since 1989 and not reported by AlphaEvolve.
\end{abstract}
